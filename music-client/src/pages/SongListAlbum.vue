    ...
    setRank(){
      ...
      if(this.loginIn){
        ...
        setRank(params)
          .then(res => {
            if(res.code == 1){
              _this.notify('评分成功','success');
              _this.getRank(this.songListId);
            }else{
              _this.notify('评分失败','error');
            }
          })
          .catch(err =>{
            _this.notify('您已经评价过啦~~~~','error');
          })
      }else{
        _this.rank = null;
        _this.notify('请先登录','warning');
      }
    }
  }
}
