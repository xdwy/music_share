...
export default {
  ...
  computed: {
    ...mapGetters([
      ...
      'duration',                //音乐时长
      'curTime',                //当前音乐的播放位置
      'showAside',         //是否显示正在播放中的歌曲列表
      'listIndex',          //当前歌曲在歌单中的位置
      'listOfSongs',         //歌单中的歌曲内容
      'autoNext',            //自动播放下一首
      'loginIn',            //用户是否登录
      'userId',              //当前登录用户的id
  ...
  methods:{
    ...
    //显示播放中的歌唱列表
    changeAside(){
      this.$store.commit('setShowAside',!this.listFlag);
    ...
    parseLyric(text){
      ...
      //遍历每一行，形成一个每行带着俩元素的数组，第一个元素是以秒为单位时间，第二个元素是歌词
      for(let item of lines){
        let time = item.match(pattern);       //存前面的时间段
        let value = item.replace(pattern,''); //存后面的歌词
        for(let item1 of time){
          let t = item1.slice(1,-1).split(":");  //取得时间 换算成数组
          if(value!=''){
